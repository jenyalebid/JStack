...
`/tag` is the same act by hand, and it **never reaches the model**: `hooks/tag-command.py` answers it at UserPromptSubmit and blocks the prompt, the way Claude Code's own `/color` takes effect without asking anything of the session. Bare lists the vocabulary with a marker on what this session carries; a carried name toggles off; a known name attaches; an unknown name mints only when trailing words give it a description. Every branch is decidable from the db, which is exactly why spending a turn on it was the wrong shape. The skill of the same name stays for the model tagging mid-turn, in light of work it has been doing. Reads (`tail`, `grep`, `recall`) take `--tag <name>` and error on an undefined one — silence there would read as "that never happened", a different and more misleading answer than "no such tag". `tag show <name>` is the cross-seat surface: one subject, every seat that touched it.
