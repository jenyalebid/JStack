...
second is keying: `jarvis-chat` and `jarvis-service-call` are seats of one agent,
...
def test_an_agent_nobody_touched_is_off():
    ...
    assert agent_prefs.is_on(FLAG, "jarvis") is False
    assert agent_prefs.prefs() == {FLAG: []}


def test_a_switch_survives_being_written_and_read():
    agent_prefs.set_flag(FLAG, "jarvis", True)
    assert agent_prefs.is_on(FLAG, "jarvis") is True
    assert agent_prefs.enabled(FLAG) == {"jarvis"}


def test_switching_back_off_removes_the_row():
    agent_prefs.set_flag(FLAG, "jarvis", True)
    agent_prefs.set_flag(FLAG, "jarvis", False)
    assert agent_prefs.is_on(FLAG, "jarvis") is False
    assert agent_prefs.prefs() == {FLAG: []}
...
def test_one_agents_switch_is_not_anothers():
    agent_prefs.set_flag(FLAG, "jarvis", True)
    assert agent_prefs.is_on(FLAG, "mario") is False


@pytest.mark.parametrize("seat", ["jarvis-chat", "jarvis-service-call", "JARVIS-Chat"])
def test_every_seat_of_an_agent_shares_the_switch(seat):
    ...
    assert agent_prefs.is_on(FLAG, "jarvis") is True
    assert agent_prefs.is_on(FLAG, "jarvis-social-chat") is True


@pytest.mark.parametrize("flag,agent", [
    pytest.param("compact_when_don", "jarvis", id="a-typo-is-not-a-flag"),
    pytest.param("anything_at_all", "jarvis", id="unknown-flag"),
...
def test_an_unknown_flag_reads_as_on_for_nobody():
    ...
    assert agent_prefs.is_on("no_such_flag", "jarvis") is False
...
def test_get_answers_every_flag_and_who_it_is_on_for(client):
    agent_prefs.set_flag(FLAG, "jarvis", True)
    assert client.get(BASE).json() == {FLAG: ["jarvis"]}
...
def test_post_flips_it_and_answers_the_whole_state(client):
    ...
    body = client.post(BASE, json={"agent_id": "jarvis-chat", "flag": FLAG, "on": True})
    assert body.status_code == 200
    assert body.json() == {FLAG: ["jarvis"]}
    assert agent_prefs.is_on(FLAG, "jarvis") is True

    off = client.post(BASE, json={"agent_id": "jarvis-chat", "flag": FLAG, "on": False})
...
def test_an_unknown_flag_is_a_400(client):
    ...
    r = client.post(BASE, json={"agent_id": "jarvis", "flag": "nope", "on": True})
