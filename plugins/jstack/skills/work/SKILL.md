...
### 1 — Orient
`date`. Your seat's recent timeline entries are injected on entry — that is your cross-session context; for more history run `log_event tail <agent>/<submode> -n 20`.
...
End ready to execute. Do **not** change code or open a PR — `/work` is preparation. The actual work is whatever the user asks next, now that you're grounded.
