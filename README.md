...
And the supporting machinery: 17 path-scoped rule files (auto-load by glob after install), a **PreToolUse hook** that re-injects path-matched rules at edit time even when the file lives outside the session's launch tree, 12 bundled `bin/` adapters (`open-terminal-here`, `file-followup`, `file-issue`, `place-issue`, `task-create`, `log_event`, `msg`, `session-review-spawn`, `session-files`, `dub-session`, `open-artifact`, `pict`), a `systems.json` registry where every bundled system declares a runnable test (`plugins/jstack/tests/*.sh` — run them any time), and per-system deep docs under `plugins/jstack/docs/systems/`.
